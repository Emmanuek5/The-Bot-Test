...
const {EmbedBuilder ,ChannelType, PermissionsBitField, ButtonInteraction, ChatInputCommandInteraction} = require('discord.js');
        ...
        if (subcommand === 'create') {
            ...
            if (!ticket) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setAuthor({name: interaction.user.tag, iconURL: interaction.user.avatarURL()})
                    .setFooter({text: interaction.guild.name, iconURL: interaction.guild.iconURL()});
                const channel = await interaction.member.guild.channels.cache.find(c => c.id === ticket.channelID);
                channel.delete();
