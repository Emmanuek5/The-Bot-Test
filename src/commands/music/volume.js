const { EmbedBuilder, SlashCommandBuilder, PermissionsBitField, PermissionFlagsBits, VoiceChannel, GuildEmoji } = require('discord.js');
...
async execute(interaction, client,args) {
    ...
    if (volume > 100 || volume < 0) {
        embed.setTitle('Volume must be between 0 and 100')
            .setColor('Red')
            .setAuthor({ name: 'The Blue Bot', iconURL: process.env.BOT_AVATAR })
            .setTimestamp()
        return interaction.reply({ embeds: [embed] })
    ...
    embed.setTitle(`Volume set to ${volume}`)
        .setColor('Purple') 
        .setAuthor({ name: 'The Blue Bot', iconURL: process.env.BOT_AVATAR })
        .setTimestamp()
    return interaction.reply({ embeds: [embed] })
