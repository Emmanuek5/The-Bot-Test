...
module.exports = {
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option
            .setName("playlist")
            .setDescription("The Playlist To Add The Song To")
            .setRequired(true)
        )
        .addStringOption((option) =>
          option
  ...
  async execute(interaction, client, args) {
    ...
    const option = options.getString("name");
    ...
    if (subcommand == "list") {
      ...
      if (playlists.length < 1) {
        const embed = new EmbedBuilder().setDescription(
          `${member}' You Dont Have Any Playlists`
        );

        await interaction.editReply({ embeds: [embed] });
      }

      const embed = new EmbedBuilder().setDescription(`${member}'s Playlist/s`);
      playlists.forEach((playlist) => {
        embed.addFields({
          name: playlist.name,
          value: `Duration:${playlist.duration.toString()}`,
        });
      });
    ...
    if (subcommand == "load") {
      ...
      if (playlist.songs < 1) {
        await interaction.editReply({
          content: "Please Put Songs In That Playlist :musical_note:",
        });
      }
    ...
    if (subcommand == "add-song") {
      ...
      const playlist = user.playlist.find(
        (x) => x.name.toLowerCase() == playlisttoadd.toLowerCase()
      );
      if (!playlist) {
        await interaction.editReply({
          content: "You Don't Have A Playlist With That Name :sob:",
