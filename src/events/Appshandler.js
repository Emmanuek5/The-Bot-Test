...
const serverdata = ServerSchema.findOne({guildId: guild.id})
    if (commandName == "Report") {
      ...
      .setTitle("Report a user").setCustomId("reportUserModal")
      .addComponents(
        new ActionRowBuilder().setComponents(
            new TextInputBuilder().setCustomId("reportMessage").setLabel("report Message").setStyle(TextInputStyle.Paragraph).setMaxLength(500).setMinLength(10).setRequired(true)
        )
      )
      await interaction.showModal(modal)
   const modalSubmit = await interaction.awaitModalSubmit({filter: (i)=>{
 return true;
      }, time: 100000 })
 const Reason = modalSubmit.fields.getTextInputValue("reportMessage");
      ...
      if (reports.reportCount > 9) {
           if (
             ...
             interaction.reply({ content: "This User Has Been Reported 10 Times But I Cannot Mute Them :sob:" });
             return;
           }
      }
      const options = {
