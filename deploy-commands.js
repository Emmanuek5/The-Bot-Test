...
const otherFiles = fs
  .readdirSync("src/commands/other")
  .filter((file) => file.endsWith(".js"));
...
const moderationFiles = fs.readdirSync('src/commands/moderation/roles').filter(file => file.endsWith('.js'));
...
(async () => {
    try {
        ...
        console.log(`Successfully reloaded application (/) commands. Number of commands: ${commands.length}`);
    } catch (error) {
        console.error(error);
    }
})();